...
import CoordinadorCard from '../../../components/Coordinador/CoordinadorCard';
...
export default function CoordinadorList() {
            ...
            Alert.alert(
                ...
                [
                    { text: 'Cancelar', style: 'cancel' },
                    {
                        ...
                        onPress: async () => {
                            try {
                                const ok = await removeCoordinador(id);
                                if (ok) {
                                    Alert.alert('Éxito', 'Coordinador eliminado correctamente.');
                                }
                            } catch (error) {
    ...
    const handleSearch = useCallback((text: string) => {
        setSearchText(text);
        if (text.trim()) {
            fetchCoordinadores(1, text);
        } else {
            fetchCoordinadores(1);
        }
    }, [setSearchText, fetchCoordinadores]);
    ...
    const statsData = useMemo(() => {
        const total = pagination?.total ?? coordinadores.length ?? 0;
        const active = coordinadores.filter(c => (c.user?.name || c.name) && c.status === 'active').length;
        const inactive = coordinadores.filter(c => (c.user?.name || c.name) && c.status === 'inactive').length;

        return { total, active, inactive };
    }, [pagination, coordinadores]);
            ...
            <View style={[styles.header, isDark && { backgroundColor: '#1F2937' }]}>
                ...
                <View style={styles.titleSection}>
                    ...
                    <Text style={[styles.subtitle, isDark && { color: '#D1D5DB' }]}>
                        {searchText ? `Resultados para "${searchText}"` : 'Gestiona tus coordinadores'}
                    </Text>
                </View>
                    ...
                    <View style={[styles.statsCard, styles.statsCardMain]}>
                        ...
                        <View style={styles.statsContent}>
                            ...
                            <Text style={styles.statLabel}>
                                {searchText ? 'Encontrados' : 'Total coordinadores'}
                            </Text>
                        </View>
                    </View>
    ...
    const renderEmpty = useCallback(() => {
        ...
        const isSearchEmpty = searchText && coordinadores.length === 0;
        ...
        return (
            <View style={styles.emptyContainer}>
                ...
                <Text style={styles.emptyTitle}>
                    {isSearchEmpty ? 'Sin resultados' : 'No hay coordinadores'}
                </Text>

                <Text style={styles.emptySubtitle}>
                    {isSearchEmpty
                        ? `No encontramos coordinadores que coincidan con "${searchText}"`
                        : 'Comienza agregando el primer coordinador de tu equipo'}
                </Text>
                ...
                {!searchText ? (
                    ...
                    >
                        ...
                        <Text style={styles.emptyButtonText}>Crear primer coordinador</Text>
                    </TouchableOpacity>
                ) : (
                    <TouchableOpacity
    ...
    }, [loading, searchText, coordinadores.length, goToCreate, clearSearch]);

    const renderFooter = useCallback(() => {
        if (!pagination || coordinadores.length === 0) return null;
        return (
    ...
    }, [pagination, changePage, paginationLoading, coordinadores.length, isDark]);

    const renderCoordinadorItem = useCallback(({ item }: { item: Coordinador }) => (
        <CoordinadorCard
            item={item}
            onPress={goToDetail}
            ...
            <SafeAreaView style={[styles.container, { backgroundColor: isDark ? '#111827' : '#F8FAFC' }]}>
                ...
                <View style={styles.loadingContainer}>
                    ...
                    <Text style={[styles.loadingText, isDark && { color: '#D1D5DB' }]}>
                        {isSearching ? 'Buscando coordinadores...' : 'Cargando coordinadores...'}
                    </Text>
                </View>
            </SafeAreaView>
        );
    ...
    return (
        <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
            ...
            <FlatList
                ...
                renderItem={renderCoordinadorItem}
                ListHeaderComponent={renderHeader}
                ListEmptyComponent={renderEmpty}
                ListFooterComponent={renderFooter}
                contentContainerStyle={[
                    styles.listContent,
                    coordinadores.length === 0 && { flex: 1 }
                ]}
                onRefresh={onRefresh}
                ...
                accessibilityLabel="Lista de coordinadores"
            ...
            <View style={styles.fabContainer} pointerEvents="box-none">
                <TouchableOpacity
                    ...
                    onPress={goToCreate}
                    accessibilityLabel="Crear coordinador"
                    accessibilityRole="button"
                    activeOpacity={0.8}
                >
